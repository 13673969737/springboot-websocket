...
@ServerEndpoint(value = "/mapSocket/{userno}")
@Component
public class MyWebSocket {
    ...
    public void onMessage(String message, Session session) {
        ...
        String[] AllOrSingle = message.split("[|]");//
        System.out.println(AllOrSingle.length);
        if (AllOrSingle.length==1) {//群发消息
            sendAll(message);
        } else {//给指定的人发消息
            sendToUser(message);
        }
        ...
        for (String key : webSocketSet.keySet()) {
            try {
                ...
                if (!userno.equals(key)) {
                    webSocketSet.get(key).sendMessage(now + "用户 <span style='color:red;'>" + userno + "</span> 发来消息：" + " <br/> " + sendMessage);
